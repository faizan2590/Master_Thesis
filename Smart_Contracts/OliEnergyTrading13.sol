        ...
        for (var v = 0; v <= maxRate; v++) {
            if (SRate[v] >= DRate[v]) {
                ...
                for (var q = 0; q < _producer.length; q++){
                    if(GenBid[_producer[q]].rate <= v) {
                        //Reward=rate-gFee
                        coin.set_OliCoinBalance(_producer[q], int32(((GenBid[_producer[q]].rate-dgfee.get_dGFee(_producer[q])) * GenBid[_producer[q]].amount)/4));
                        //GSO reward
                        coin.set_OliCoinBalance(origin.get_gsoAddr(origin.get_oliTrafoid(_producer[q])), int32(dgfee.get_dGFee(_producer[q])*GenBid[_producer[q]].amount));
...
contract BilateralTrading is mortal{
    ...
    function biTrade() {
        ...
        //producer coins
        coin.set_OliCoinBalance(msg.sender, int32((availStock[msg.sender].amount*availStock[msg.sender].minMaxRate)/400));
        //consumer coins
        coin.set_OliCoinBalance(availStock[msg.sender].higestBidder, -int32(((availStock[availStock[msg.sender].higestBidder].amount)*(availStock[msg.sender].minMaxRate))/400));
    }
